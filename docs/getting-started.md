...
先安装 Git、Python venv、Node.js 22 和 FFmpeg。Debian/Ubuntu 系统组件示例：

```bash
sudo apt-get update
sudo apt-get install -y git python3-venv ffmpeg fonts-wqy-zenhei libgomp1 libsndfile1
```
...
首次安装会下载较大的依赖、Chromium 和模型，请预留数 GB 磁盘空间并保持网络畅通。
...
等待终端显示“ClipTalk 已就绪”，然后在页面“设置”中配置视觉模型、剪辑规划和 AI 助手所需的模型服务。

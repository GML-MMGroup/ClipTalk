    ...
    if len(segments) <= 16:
        for count in range(1, len(segments) + 1):
            for indices in itertools.combinations(range(len(segments)), count):
                ...
                quality = sum(_number(candidate_map.get(_candidate_id(item), item).get("score"), 50) for item in chosen)
                essential = sum(bool(item.get("essential")) for item in chosen)
                score = (
                    len(events) * 16.0 + quality * .08 + essential * 3.0
                    - abs(target - duration) * 2.2
    ...
    *,
    ...
    if allow_fill and target and total() < target - max(4.0, target * .1):
        ...
        ranked = sorted(pool, key=lambda item: (
            ...
            -_number(item.get("score")), _number(item.get("start")),
        ))
